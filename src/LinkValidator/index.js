...
const GUIDE_OR_PAGE_LINKS = new RegExp(/(guide|guides|g|page|pages|c|changelog|[^a-z]p):\/\/([a-z-_0-9.\/]*)/, 'ig')
...
class LinkValidator {
  ...
  guideOrPageLinks(contentFolder)  {
    // map over all items
    return [...this.content.matchAll(GUIDE_OR_PAGE_LINKS)]
      // get the actual captured part (the url)
      .map(match => {
        if (match[1].startsWith('g')) {
          return ({
            original: match[0], 
            resolved: path.join(contentFolder, `guides`, match[2])
          })
        } else if (match[1].startsWith('p')) {
          return ({
            original: match[0], 
            resolved: path.join(contentFolder, `pages`, match[2])
          })
        } else if (match[1].startsWith('c')) {
          return ({
            original: match[0], 
            resolved: path.join('content', match[2])
          })
        }
      })
  }
