...
* **`classification`:** IE11で分類のツールヒントが表示されない ([#1710][38]) ([`d08b53c`][39])
...
* **`floatfield`:** 浮動小数点フィールドの末尾のピリオドを常に許可 ([#1178][86]) ([`61d4e3d`][87])
* **`footer-indicator`:** フッターインジケータの上に表示されるようにツールヒントを移動 ([`bf7782c`][88])
...
* **`pill-selector`:** 入力をトラップする`onInput`を公開 ([#1450][147]) ([`48d22a4`][148])
* **`pill-selector`:** UXの微調整 ([#1315][149]) ([`e4fdf58`][150])
* **`pill-selector-dropdown`:** `PillSelector`が無効な場合にピルを無効化 ([#1357][151]) ([`2bf496e`][152])
...
* **`task-form`:** 無効な必須フィールドが残っている場合のエラーメッセージを更新 ([#1353][211]) ([`be10f37`][212])
...
* **`text-input`:** ツールヒントの切り替え時にフォーカスが失われる ([#1185][247]) ([`8a4c294`][248])
...
* 長いテキストにカーソルを合わせたときにツールヒントを表示 ([#1230][307]) ([`c41dd47`][308])
...
* **`unified-share-modal`:** 招待セクションのツールヒントの変更 ([#1499][499]) ([`d7730c7`][500])、[#1498][378]をクローズ
* **`unified-share-modal`:** 招待セクションのツールヒントの変更 ([#1504][501]) ([`b9486bc`][502])
...
* **`validators`:** ホスト、`ipv4`、およびドメイン名の検証機能 ([#1212][517]) ([`f653dac`][518])
...
* **`deps`:** ピア依存関係が更新されました。NPM警告を回避するには、ホストアプリケーションも更新する必要があります。
