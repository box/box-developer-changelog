...
* **usm:** コラボレータリストのDOMでアイコンの前に人物を指定 ([#3306][60]) ([`32c656d`][61])
...
* USMおよび共有リンクコンポーネントの`bdl-gray-62`を置換 ([#3289][68]) ([`1f25b98`][69])
...
* **sub-header:** 選択コンポーネントと更新テストを使用するように並べ替えを更新 ([#3239][78]) ([`528a157`][79])
...
* **content-explorer:** `MoreOptions`ボタンの`aria-label`を追加 ([#3197][101]) ([`f4b69f0`][102])
* **content-explorer:** グリッドのスライダに`aria-label`を追加 ([#3189][103]) ([`e3900b3`][104])
* **content-explorer:** 何も存在しない場合にデフォルトの共有リンクを作成 ([#3209][105]) ([`5d05a2b`][106])
* **content-explorer:** `modalContainer`のすべてのプロパティを`Modal`に渡す ([#3104][107]) ([`5d5b314`][108])
...
* **content-sidebar:** スレッド形式のフィード項目の選択状態とカーソルを合わせた状態 ([#3173][117]) ([`a7dc6a9`][118])
...
* **usm:** `aria-label`をツールチップを閉じるボタンに追加 ([#3211][189]) ([`529bf10`][190])
...
* **annotation-thread:** 応答フォーム、UI、その他軽度の修正を追加 ([#3157][206]) ([`55ca058`][207])
* **annotation-thread:** 注釈スレッドのコメントのビジュアルの修正 ([#3128][208]) ([`fac5133`][209])
...
* **content-sidebar:** 解決済みのコメント/注釈で「編集済み」を非表示にする ([#3152][250]) ([`07f5fc7`][251])
* **content-sidebar:** スレッド形式のフィード項目の選択状態とカーソルを合わせた状態 ([#3162][252]) ([`a6c125d`][253])
...
* **list-view:** リストビュー項目のカーソルを合わせた状態の変更 ([#3115][260]) ([`103a5d1`][261])
